 ...
 *
 * Modified by Uber by copying from original file SqlBase.g4.
 * Modifications copyright (C) 2017 Uber
 *
 */


grammar UberScriptQuerySql;
...
root
    : ((fileAssignment ';') | (jsonQueryStatementAssignment ';') | (statementAssignment ';'))* (actionStatement ';')* EOF
    ;

jsonQueryStatementAssignment
    : statementAssignment_tableAlias EQ JSON statementAssignment_queryEngine jsonQueryStatementAssignment_query
    ;
    
jsonQueryStatementAssignment_query
    : configSetting* jsonObject
    ;
    
statementAssignment
    : statementAssignment_tableAlias EQ (statementAssignment_queryType statementAssignment_queryEngine)? statementAssignment_query
    ;
...
fileAssignment
    : fileAssignment_tableAlias EQ 'FILE' fileAssignment_fileType fileAssignment_fileLocation
    ;
fileAssignment_tableAlias
    : IDENTIFIER
    ;
fileAssignment_fileType
    : JSON | TEXT | CSV | PARQUET
    ;
...
actionStatement
    : qualifiedName '(' ')' | qualifiedName '(' actionStatement_param (',' actionStatement_param)* ')'
    ;
...
// TODO better using a real syntax for url
URL_STRING
    : 'H' 'D' 'F' 'S' ':' '/' '/' ( ~(' '|';') )+
    | 'H' 'T' 'T' 'P' ':' '/' '/' ( ~(' '|';') )+
    | 'F' 'I' 'L' 'E' ':' '/' '/' ( ~(' '|';') )+
...
// Following is copied from Spark SQL
